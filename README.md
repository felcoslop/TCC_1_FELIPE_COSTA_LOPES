...
- **Filtro EMI (Sanidade Elétrica)**: Zero automático de RPM/Corrente se houver discrepância física (RPM alto sem consumo elétrico compatível), filtrando interferências eletromagnéticas.
...
- **Verificação de Confiabilidade**: Substituição do limite relativo (30%) por um limite **absoluto de 1.000 registros válidos** de Corrente/RPM, permitindo a classificação correta de equipamentos elétricos com baixo duty-cycle.
...
- **Tratamento de Erros**: Sistema robusto para problemas de conexão e resiliência a erros de encoding (UTF-8/CP1252) com logs de erro descritivos no console.
...
├── models/
│   └── c_{{m_mpoint}}/                         # Modelos e configurações
...
├── results/
│   └── c_{{m_mpoint}}/                         # Visualizações e Relatórios
...
├── logs/                                  # Logs de execução por etapa
...
├── scripts/                               # Módulos de processamento
│   ├── baixar_*.py                        # Scripts de coleta
│   ├── processar_dados_simples*.py        # Limpeza e interpolação
│   ├── unir_sincronizar_periodos*.py      # União de sensores
│   ├── normalizar_dados_kmeans*.py        # Preparo para K-means
│   ├── kmeans_classificacao*.py           # Treino e classificação
│   ├── analise_intervalo_completa*.py     # Injeção e análise
│   └── plot_vibracao_temporal_colorido.py # Visualização avançada
