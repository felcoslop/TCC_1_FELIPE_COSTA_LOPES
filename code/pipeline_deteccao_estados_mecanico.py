...
class PipelineDeteccaoEstadosMecanico:
    ...
    def executar_processamento_mecanico(self, mpoint=None):
        ...
        for script, descricao in scripts:
            ...
            try:
                ...
                self.execution_times_minutes[script] = (end_time - start_time) / 60.0
    ...
    def executar_visualizacao_3d_mecanico(self, mpoint=None):
        ...
        try:
            ...
            self.execution_times_minutes['visualizar_clusters_3d_mecanico.py'] = (end_time - start_time) / 60.0
    ...
    def abrir_graficos_gerados(self, mpoint):
        ...
        dir_resultados = dirs_mpoint['results']
        
        # Gráfico de normalização
        grafico_norm = dir_resultados / f'dados_normalizados_analise_{mpoint}_mecanico.png'
        if grafico_norm.exists():
            arquivos.append(grafico_norm)
        
        # Gráfico de K-means
        grafico_kmeans = dir_resultados / f'analise_kmeans_clusters_mecanico_{mpoint}.png'
        if grafico_kmeans.exists():
            arquivos.append(grafico_kmeans)
        
        # Gráfico 3D
        grafico_3d = dir_resultados / f'estados_temperatura_vibracao_tempo_3d_1_{mpoint}.png'
        if grafico_3d.exists():
            arquivos.append(grafico_3d)
            
        # Gráfico de Vibração Temporal Reconstruída
        grafico_vibracao = dir_resultados / f'vibracao_reconstrucao_{mpoint}.png'
        if grafico_vibracao.exists():
            arquivos.append(grafico_vibracao)
        ...
        for arquivo in arquivos:
            try:
                # Obter o caminho absoluto (resolve problemas de path relativo no os.startfile)
                caminho_abs = str(arquivo.resolve())
                if sistema == 'Windows':
                    os.startfile(caminho_abs)
                elif sistema == 'Darwin':  # macOS
                    subprocess.run(['open', caminho_abs])
                else:  # Linux
                    subprocess.run(['xdg-open', caminho_abs])
    ...
    def modo_treino_mecanico(self, mpoint=None):
        ...
        enrich_results_file(mpoint, {
            ...
            'mpoint': mpoint,
            'execution_times_minutes': self.execution_times_minutes
        })
