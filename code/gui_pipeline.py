...
from dotenv import load_dotenv
        ...
        # Deixa a interface com tema claro e azul (Institucional)
...
class PipelineGUI:
    def __init__(self):
        ...
        # Estilo global para ttk (afeta o Scrollbar do Listbox e DateEntry)
        ...
        # Configurar tamanho padrão da janela 1024x768
        self.root.geometry("1024x848")
        self.root.minsize(1024, 848)
        # Centralizar na tela
        self.root.update_idletasks()
        width = 1024
        height = 768
        x = (self.root.winfo_screenwidth() // 2) - (width // 2)
        y = (self.root.winfo_screenheight() // 2) - (height // 2) - 30 # reservando espaço para barra de tarefas
        self.root.geometry(f'{width}x{height}+{x}+{y}')
        
        # Permitir maximizar mas iniciar em janela
        # self.root.attributes('-fullscreen', True) # Removido conforme pedido
    ...
    def criar_interface(self):
        ...
        # Inicializar estados dos combos e botões
        self.atualizar_combo_mpoints()
    
    def criar_aba_treino(self, parent):
        """Cria interface da aba de treino"""
        # Frame de fundo (div que fica por baixo)
        content_frame = self.criar_frame(parent, fg_color="white")
        content_frame.grid(row=0, column=0, sticky="nsew")
        parent.grid_rowconfigure(0, weight=1)
        parent.grid_columnconfigure(0, weight=1)

        # Formato de Computador (Horizontal): 2 colunas
        content_frame.grid_rowconfigure(0, weight=1)
        content_frame.grid_columnconfigure(0, weight=1) # Controles
        content_frame.grid_columnconfigure(1, weight=2) # Console (maior)

        # Painel Esquerdo (Controles)
        left_panel = self.criar_frame(content_frame, fg_color="transparent")
        left_panel.grid(row=0, column=0, sticky="nsew", padx=10, pady=10)
        left_panel.grid_columnconfigure(0, weight=1)

        # Header
        header_frame = self.criar_frame(left_panel, fg_color="transparent")
        header_frame.grid(row=0, column=0, sticky="ew", pady=(0, 10))

        self.criar_label(header_frame, text="Treinar Modelo K-means", font=("Arial", 18, "bold")).pack(anchor="w")
        self.criar_label(header_frame, text="Processar dados históricos e gerar parâmetros dinâmicos", font=("Arial", 12), text_color="gray").pack(anchor="w", pady=5)
        
        # Container de mpoints
        mpoints_frame = self.criar_frame(left_panel, fg_color="#f5f5f5", border_width=1, border_color="#d0d0d0")
        mpoints_frame.grid(row=1, column=0, sticky="ew", pady=10)
        mpoints_frame.grid_columnconfigure(0, weight=1)

        self.criar_label(mpoints_frame, text="Selecione o Equipamento:", font=("Arial", 14, "bold"), text_color="#333333").grid(row=0, column=0, pady=(15, 5), sticky="w", padx=15)
        
        # ComboBox para selecionar mpoint (Substituindo a Listbox)
        self.mpoint_treino_combo = self.criar_combo(
            mpoints_frame, 
            values=["Carregando..."],
            width=300
        )
        self.mpoint_treino_combo.grid(row=1, column=0, padx=15, pady=10, sticky="we")

        # Botões
        btn_frame = self.criar_frame(mpoints_frame, fg_color="transparent")
        btn_frame.grid(row=2, column=0, pady=(10, 20), sticky="ew", padx=15)

        self.btn_atualizar_treino = self.criar_button(
            btn_frame, text="Atualizar Lista", command=self.atualizar_lista_mpoints,
            height=40,
            fg_color="#e0e0e0", text_color="black", hover_color="#d0d0d0"
        )
        self.btn_atualizar_treino.pack(side="left", fill="x", expand=True, padx=(0, 5))

        self.btn_iniciar_treino = self.criar_button(
            btn_frame, text="Iniciar Treino", command=self.executar_treino,
            height=40,
            fg_color="#1f538d", text_color="white", hover_color="#14375e" # Azul institucional, sem verde limão
        )
        self.btn_iniciar_treino.pack(side="right", fill="x", expand=True, padx=(5, 0))
        
        # Painel Direito (Console)
        right_panel = self.criar_frame(content_frame, fg_color="#f5f5f5", border_width=1, border_color="#d0d0d0")
        right_panel.grid(row=0, column=1, sticky="nsew", padx=10, pady=10)
        right_panel.grid_rowconfigure(1, weight=1)
        right_panel.grid_columnconfigure(0, weight=1)

        self.criar_label(right_panel, text="Console de Saída (Treino):", font=("Arial", 12, "bold"), text_color="#333333").grid(row=0, column=0, sticky="w", padx=10, pady=(10, 5))

        # Usar CTkTextbox que tem scrollbars bonitas e cinzas integradas nativamente
        self.console_treino = ctk.CTkTextbox(
            right_panel, font=("Consolas", 11),
            fg_color="white", text_color="#333333",
            scrollbar_button_color="#888888", scrollbar_button_hover_color="#555555"
        )
        self.console_treino.grid(row=1, column=0, sticky="nsew", padx=10, pady=(0, 10))

        # Carregar lista inicial
        self.atualizar_lista_mpoints()
    
    # Note: atualizar_display_datas removed as we don't need external labels anymore
    
    def criar_aba_analise(self, parent):
        """Cria interface da aba de análise por intervalo em 2 colunas"""
        # Frame de fundo (div que fica por baixo)
        content_frame = self.criar_frame(parent, fg_color="white")
        content_frame.grid(row=0, column=0, sticky="nsew")
        parent.grid_rowconfigure(0, weight=1)
        ...
        content_frame.grid_rowconfigure(0, weight=1)
        content_frame.grid_columnconfigure(0, weight=1) # Controles
        content_frame.grid_columnconfigure(1, weight=2) # Console
        ...
        self.criar_label(header_frame, text="Análise por Intervalo", font=("Arial", 18, "bold")).pack(anchor="w")
        self.criar_label(header_frame, text="Classificar estados usando modelo treinado e thresholds dinâmicos", font=("Arial", 12), text_color="gray").pack(anchor="w", pady=5)

        # Configurações de conexão InfluxDB
        config_frame = self.criar_frame(left_panel, fg_color="#f5f5f5", border_width=1, border_color="#d0d0d0")
        config_frame.pack(fill="x", pady=5)
        config_frame.grid_columnconfigure(1, weight=1)
        config_frame.grid_columnconfigure(3, weight=1)

        self.criar_label(config_frame, text="Configuração InfluxDB", font=("Arial", 14, "bold"), text_color="#333333").grid(row=0, column=0, columnspan=4, pady=10, sticky="w", padx=10)
        
        self.criar_label(config_frame, text="IP:", font=("Arial", 12), text_color="#333333").grid(row=1, column=0, padx=(10, 5), pady=5, sticky="e")
        self.ip_entry = self.criar_entry(config_frame)
        self.ip_entry.insert(0, self.influxdb_ip)
        self.ip_entry.grid(row=1, column=1, padx=5, pady=5, sticky="we")

        self.criar_label(config_frame, text="Porta:", font=("Arial", 12), text_color="#333333").grid(row=1, column=2, padx=(10, 5), pady=5, sticky="e")
        self.porta_entry = self.criar_entry(config_frame, width=60)
        self.porta_entry.insert(0, self.influxdb_port)
        self.porta_entry.grid(row=1, column=3, padx=5, pady=5, sticky="w")
        
        # Seleção de Mpoint
        mpoint_frame = self.criar_frame(left_panel, fg_color="#f5f5f5", border_width=1, border_color="#d0d0d0")
        mpoint_frame.pack(fill="x", pady=5)

        self.criar_label(mpoint_frame, text="Equipamento Treinado:", font=("Arial", 14, "bold"), text_color="#333333").grid(row=0, column=0, pady=10, sticky="w", padx=10)

        self.mpoint_analise_var = tk.StringVar()
        mpoints_treinados = self.listar_mpoints_treinados()
        if not mpoints_treinados:
            mpoints_treinados = ["Não existe equipamento treinado ainda"]
        
        self.mpoint_analise_combo = self.criar_combo(mpoint_frame, values=mpoints_treinados, width=220)
        self.mpoint_analise_combo.set(mpoints_treinados[0])
        self.mpoint_analise_combo.grid(row=0, column=1, padx=5, pady=10, sticky="w")

        self.criar_button(
            mpoint_frame, text="Atualizar", command=self.atualizar_combo_mpoints, width=80,
            fg_color="#e0e0e0", text_color="black", hover_color="#d0d0d0"
        ).grid(row=0, column=2, padx=5)
        
        # Seleção de datas
        datas_frame = self.criar_frame(left_panel, fg_color="#f5f5f5", border_width=1, border_color="#d0d0d0")
        datas_frame.pack(fill="x", pady=5)

        self.criar_label(datas_frame, text="Intervalo de Análise (GMT-3):", font=("Arial", 14, "bold"), text_color="#333333").grid(row=0, column=0, columnspan=4, pady=10, sticky="w", padx=10)

        # Configuração Comum de Cores para o Calendário Institucional
        cal_style = {
            'background': 'white', 'foreground': 'black', 'borderwidth': 1, 'bordercolor': '#cccccc',
            'headersbackground': '#e0e0e0', 'headersforeground': 'black',
            'normalbackground': 'white', 'normalforeground': 'black',
            'selectbackground': '#d0d0d0', 'selectforeground': 'black',
            'weekendbackground': '#f9f9f9', 'weekendforeground': 'black',
            'othermonthforeground': '#aaaaaa', 'othermonthbackground': 'white',
            'othermonthweforeground': '#aaaaaa', 'othermonthwebackground': '#f9f9f9',
            'font': ("Arial", 11)
        }

        # Data início (Normal, sem label extra)
        self.criar_label(datas_frame, text="Início:", font=("Arial", 12), text_color="#333333").grid(row=1, column=0, padx=(10, 5), pady=5, sticky="e")
        self.data_inicio_cal = DateEntry(
            datas_frame, width=12, date_pattern='dd-mm-yyyy',
            showweeknumbers=False, firstweekday='sunday',
            locale='pt_BR',
            maxdate=datetime.now().date(), mindate=datetime(2025, 1, 1).date(), **cal_style
        )
        self.data_inicio_cal.grid(row=1, column=1, padx=5, pady=5, sticky="w")

        self.criar_label(datas_frame, text="Hora:", font=("Arial", 12), text_color="#333333").grid(row=1, column=2, padx=(10, 5), pady=5, sticky="e")
        self.hora_inicio_entry = self.criar_entry(datas_frame, width=90, placeholder_text="HH:MM:SS")
        self.hora_inicio_entry.insert(0, "00:00:00")
        self.hora_inicio_entry.grid(row=1, column=3, padx=5, pady=5, sticky="w")

        # Data fim (Normal, sem label extra)
        self.criar_label(datas_frame, text="Fim:", font=("Arial", 12), text_color="#333333").grid(row=2, column=0, padx=(10, 5), pady=5, sticky="e")
        ...
        self.data_fim_cal.grid(row=2, column=1, padx=5, pady=5, sticky="w")

        self.criar_label(datas_frame, text="Hora:", font=("Arial", 12), text_color="#333333").grid(row=2, column=2, padx=(10, 5), pady=5, sticky="e")
        self.hora_fim_entry = self.criar_entry(datas_frame, width=90, placeholder_text="HH:MM:SS")
        self.hora_fim_entry.insert(0, "23:59:59")
        self.hora_fim_entry.grid(row=2, column=3, padx=5, pady=5, sticky="w")
        
        # Botão de análise
        btn_analise_frame = self.criar_frame(left_panel, fg_color="transparent")
        btn_analise_frame.pack(fill="x", pady=15)

        self.btn_executar_analise = self.criar_button(
            btn_analise_frame, text="Executar Análise", command=self.executar_analise,
            height=40, fg_color="#1f538d", text_color="white", hover_color="#14375e" # Azul institucional, sem verde limão
        )
        self.btn_executar_analise.pack(fill="x", padx=10)
        ...
        self.criar_label(right_panel, text="Console de Saída (Análise):", font=("Arial", 12, "bold"), text_color="#333333").grid(row=0, column=0, sticky="w", padx=10, pady=(10, 5))

        self.console_analise = ctk.CTkTextbox(
            right_panel, font=("Consolas", 11),
            fg_color="white", text_color="#333333",
            scrollbar_button_color="#888888", scrollbar_button_hover_color="#555555"
        )
        self.console_analise.grid(row=1, column=0, sticky="nsew", padx=10, pady=(0, 10))
    ...
    def criar_aba_visualizar(self, parent):
        ...
        # Header
        header_frame = self.criar_frame(content_frame, fg_color="transparent")
        ...
        # Seleção de mpoint
        select_frame = self.criar_frame(content_frame, fg_color="transparent")
        ...
        self.mpoint_viz_combo = self.criar_combo(
            select_frame,
            values=mpoints_treinados,
            width=220
        )
        self.mpoint_viz_combo.set(mpoints_treinados[0])
        self.mpoint_viz_combo.grid(row=0, column=1, padx=10, pady=10, sticky="w")

        # Botões (Direto no content_frame para evitar div cinza)
        self.btn_gerar_viz = self.criar_button(
            content_frame,
        ...
        self.btn_gerar_viz.grid(row=2, column=0, pady=20, sticky="n")

        # Console (Painel Cinza igual aos outros)
        console_frame = self.criar_frame(content_frame, fg_color="#f5f5f5", border_width=1, border_color="#d0d0d0")
        ...
        self.console_viz = ctk.CTkTextbox(
            console_frame, font=("Consolas", 11),
            fg_color="white", text_color="#333333",
            scrollbar_button_color="#888888", scrollbar_button_hover_color="#555555"
        )
    ...
    def atualizar_lista_mpoints(self):
        """Atualiza combo de mpoints disponíveis para treino (ELÉTRICOS e MECÂNICOS)"""
        ...
        if not mpoints_completos:
            msg = "Nenhum equipamento com dados completos"
            self.mpoint_treino_combo.configure(values=[msg])
            self.mpoint_treino_combo.set(msg)
        ...
        for mp in mpoints_completos:
            ...
            lista_formatada.append(f"{mp['mpoint']} {tipo_label}")
        
        self.mpoint_treino_combo.configure(values=lista_formatada)
        
        # Verificar se o que está selecionado agora é válido
        selecionado = self.mpoint_treino_combo.get()
        if selecionado not in lista_formatada:
            self.mpoint_treino_combo.set(lista_formatada[0])
            self.btn_iniciar_treino.configure(state="normal")
        elif "Nenhum equipamento" in selecionado:
            self.btn_iniciar_treino.configure(state="disabled")
        else:
            self.btn_iniciar_treino.configure(state="normal")
    ...
    def atualizar_combo_mpoints(self):
        ...
        if not mpoints:
            msg = "Não existe equipamento treinado ainda"
            self.mpoint_analise_combo.configure(values=[msg])
            self.mpoint_viz_combo.configure(values=[msg])
            self.mpoint_analise_combo.set(msg)
            self.mpoint_viz_combo.set(msg)
            self.btn_executar_analise.configure(state="disabled")
            self.btn_gerar_viz.configure(state="disabled")
            return
            
        self.mpoint_analise_combo.configure(values=mpoints)
        self.mpoint_viz_combo.configure(values=mpoints)
        
        # Validar seleção atual para Análise
        sel_analise = self.mpoint_analise_combo.get()
        if sel_analise not in mpoints:
            self.mpoint_analise_combo.set(mpoints[0])
            self.btn_executar_analise.configure(state="normal")
        elif "Não existe" in sel_analise:
            self.btn_executar_analise.configure(state="disabled")
        else:
            self.btn_executar_analise.configure(state="normal")

        # Validar seleção atual para Visualização
        sel_viz = self.mpoint_viz_combo.get()
        if sel_viz not in mpoints:
            self.mpoint_viz_combo.set(mpoints[0])
            self.btn_gerar_viz.configure(state="normal")
        elif "Não existe" in sel_viz:
            self.btn_gerar_viz.configure(state="disabled")
        else:
            self.btn_gerar_viz.configure(state="normal")
    ...
    def listar_mpoints_disponiveis(self):
        ...
        for arq in arquivos_dados:
            ...
            if match:
                ...
                # Determinar tipo real do equipamento (considerando calibração)
                tipo_real = self.detectar_tipo_equipamento(mpoint)

                if tipo_real == 'ELETRICO' and arq_estimated.exists() and arq_slip.exists():
                    # EQUIPAMENTO ELÉTRICO (tem as 3 tabelas e sensores calibrados)
                    mpoints.append({
                        'mpoint': mpoint,
                        'completo': True,
                        'estimated': True,
                ...
                elif tipo_real == 'MECANICO' and arq_slip.exists():
                    # EQUIPAMENTO MECÂNICO (tem dados_c + slip, mesmo que tenha estimated com NaN)
                    mpoints.append({
                        'mpoint': mpoint,
                        'completo': True,
    ...
    def detectar_tipo_equipamento(self, mpoint):
        """Detecta se o equipamento é MECÂNICO ou ELÉTRICO.
        
        Lógica:
        1. Se o config do modelo já define o tipo, usa esse.
        2. Se o arquivo estimated existe, verifica se current/RPM estão calibrados
           (>30% de dados reais não-NaN). Se não estão, trata como MECÂNICO.
        3. Se o arquivo estimated não existe, é MECÂNICO.
        """
        ...
        # Método 2: Verificar arquivos raw e calibração dos sensores
        arq_estimated = self.dir_raw / f'dados_estimated_{mpoint}.csv'
        if arq_estimated.exists():
            # Verificar se current/RPM estão realmente calibrados
            try:
                import pandas as pd
                # Ler apenas as colunas necessárias (amostra para performance)
                df = pd.read_csv(arq_estimated, usecols=lambda c: c in ['current', 'rotational_speed'])
                total = len(df)
                ...
                if total > 0:
                    ...
                    if 'current' in df.columns:
                        qtd_current = df['current'].notna().sum()
                        current_ok = qtd_current >= 1000  # Pelo menos 1000 registros válidos
                    
                    if 'rotational_speed' in df.columns:
                        qtd_rpm = df['rotational_speed'].notna().sum()
                        rpm_ok = qtd_rpm >= 1000  # Pelo menos 1000 registros válidos
                    
                    if current_ok or rpm_ok:
                        return 'ELETRICO'
                    else:
                        # Sensores não calibrados - tratar como mecânico
                        print(f"  [INFO] {mpoint}: current ({qtd_current} regs) e RPM ({qtd_rpm} regs) insuficientes -> MECANICO")
                        return 'MECANICO'
            except Exception:
                # Se não conseguir verificar, assume elétrico pelo arquivo existir
                return 'ELETRICO'
        else:
            return 'MECANICO'
    ...
    def executar_treino(self):
        ...
        # Pegar seleção do combo
        linha_selecionada = self.mpoint_treino_combo.get()
        
        if not linha_selecionada or "Nenhum equipamento" in linha_selecionada or "Carregando" in linha_selecionada:
            messagebox.showwarning("Aviso", "Selecione um equipamento válido da lista")
            return

        # Extrair mpoint (formato: "c_XXX [TIPO]")
        linha_selecionada = linha_selecionada.strip()
        
        # Separar mpoint do tipo
        partes = linha_selecionada.split()
        if not partes or not partes[0].startswith('c_'):
            messagebox.showwarning("Aviso", "Selecione um mpoint válido")
            return
