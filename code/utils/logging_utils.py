...
def ensure_logs_dir(mpoint: str = None) -> Path:
    """Cria o diretorio de logs se nao existir, organizado por mpoint"""
    logs_dir = Path(__file__).parent.parent / 'logs'
    if mpoint:
        logs_dir = logs_dir / mpoint
...
def save_log(log_data: Dict[str, Any], script_name: str, mpoint: str, operation: str = None) -> Path:
    ...
    """
    logs_dir = ensure_logs_dir(mpoint)
...
def enrich_results_file(mpoint: str, new_data: Dict[str, Any]) -> None:
    ...
    """
    from utils.artifact_paths import results_dir as get_results_dir
    results_dir = get_results_dir(mpoint, create=True)
