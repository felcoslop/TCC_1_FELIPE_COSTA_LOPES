...
for i, mp in enumerate(mpoints, start=1):
    ...
    if os.path.exists(filename):
        print(f"[SKIP] {mp} ja exportado, pulando...")
        continue

    print(f"\n[INFO] [{i}/{len(mpoints)}] Coletando dados para m_point: {mp}")
    t1 = time.time()
...
# -----------------------------
print("\n[RESUMO] Resumo final:")
print(f"   [OK] m_points com dados: {total_com_dados}")
print(f"   [SKIP] m_points sem dados: {total_sem_dados}")
...
if timeouts:
    with open("m_points_timeout_slip.txt", "w") as f:
        for mp in timeouts:
            f.write(mp + "\n")
    print("[SAVE] Lista de m_points que excederam tempo salva em m_points_timeout_slip.txt")
