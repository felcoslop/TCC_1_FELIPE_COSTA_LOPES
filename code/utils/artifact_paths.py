...
def get_mpoint_dirs(mpoint: str, create: bool = False) -> Dict[str, Path]:
    ...
    dirs = {
        ...
        "normalized": BASE_DIR / "data" / "normalized" / value,
        "processed": BASE_DIR / "data" / "processed" / value,
        "raw_preenchido": BASE_DIR / "data" / "raw_preenchido" / value,
    }
...
def normalized_csv_path(mpoint: str, intervalo_arquivo: str = None) -> Path:
    value = _validate_mpoint(mpoint)
    dirs = get_mpoint_dirs(value, create=True)
    intervalo_tag = f"_{intervalo_arquivo}" if intervalo_arquivo else ""
    return dirs["normalized"] / f"dados_kmeans_{value}{intervalo_tag}.csv"
...
def normalized_numpy_path(mpoint: str, intervalo_arquivo: str = None) -> Path:
    ...
    dirs = get_mpoint_dirs(value, create=True)
    intervalo_tag = f"_{intervalo_arquivo}" if intervalo_arquivo else ""
    return dirs["normalized"] / f"dados_normalizados_completos_{value}{intervalo_tag}.npy"


def processed_unificado_path(mpoint: str) -> Path:
    value = _validate_mpoint(mpoint)
    dirs = get_mpoint_dirs(value, create=True)
    return dirs["processed"] / f"dados_unificados_final_{value}.csv"


def processed_classificado_path(mpoint: str) -> Path:
    value = _validate_mpoint(mpoint)
    dirs = get_mpoint_dirs(value, create=True)
    return dirs["processed"] / f"dados_classificados_kmeans_moderado_{value}.csv"


def processed_rotulado_path(mpoint: str) -> Path:
    value = _validate_mpoint(mpoint)
    dirs = get_mpoint_dirs(value, create=True)
    return dirs["processed"] / f"dados_kmeans_rotulados_conservador_{value}.csv"
...
def raw_preenchido_dir(mpoint: str, create: bool = False) -> Path:
    dirs = get_mpoint_dirs(mpoint, create=create)
    return dirs["raw_preenchido"]
