...
class AnalisadorIntervaloCompleto:
    ...
    def validar_rotational_speed(self):
        """Valida disponibilidade de rotational_speed e current (baseado em TEMPO)
        
        IMPORTANTE: Valores 0.0 são VÁLIDOS (representam equipamento desligado).
                   Apenas valores vazios (None/NaN) por longos períodos indicam sensor não calibrado.
        
        NOTA: Esta validação agora é informativa. Quando sensores não têm cobertura 
        suficiente, a análise continua usando apenas vibração e temperatura.
        """
        ...
        if not arquivo_estimated.exists():
            print(f"  [AVISO] Arquivo estimated nao encontrado: {arquivo_estimated.name}")
            print(f"  [INFO] Continuando sem validacao de sensores")
            self.sensores_confiaveis = {'current': False, 'rpm': False}
            return True
        ...
        self.sensores_confiaveis = {'current': True, 'rpm': True}
        
        # ========== VALIDAR ROTATIONAL_SPEED ==========
        if 'rotational_speed' not in df.columns:
            print("  [AVISO] Coluna 'rotational_speed' nao encontrada")
            self.sensores_confiaveis['rpm'] = False
        else:
            ...
            if len(df_rpm_preenchido) == 0:
                print("  [AVISO] Nenhum dado de rotational_speed encontrado (todos NaN)")
                self.sensores_confiaveis['rpm'] = False
            else:
                ...
                print(f"    Valores - Media: {rpm_mean:.1f}, Maximo: {rpm_max:.1f} RPM")
                
                if pct_rpm_registros < 30.0:
                    print(f"    [AVISO] Cobertura insuficiente ({pct_rpm_registros:.1f}% < 30%)")
                    print(f"    [INFO] RPM sera IGNORADO na classificacao (dados nao confiaveis)")
                    self.sensores_confiaveis['rpm'] = False
                else:
                    print(f"    [OK] Cobertura adequada")
        
        # ========== VALIDAR CURRENT ==========
        if 'current' not in df.columns:
            print("  [AVISO] Coluna 'current' nao encontrada")
            self.sensores_confiaveis['current'] = False
        else:
            df_current_preenchido = df[df['current'].notna()].copy()
            
            if len(df_current_preenchido) == 0:
                print("  [AVISO] Nenhum dado de current encontrado (todos NaN)")
                self.sensores_confiaveis['current'] = False
            else:
                ...
                print(f"    Valores - Media: {current_mean:.1f}, Maximo: {current_max:.1f} A")
                
                if pct_current_registros < 30.0:
                    print(f"    [AVISO] Cobertura insuficiente ({pct_current_registros:.1f}% < 30%)")
                    print(f"    [INFO] Current sera IGNORADO na classificacao (dados nao confiaveis)")
                    self.sensores_confiaveis['current'] = False
                else:
                    print(f"    [OK] Cobertura adequada")
        
        # Resumo
        if not self.sensores_confiaveis['current'] or not self.sensores_confiaveis['rpm']:
            print(f"\n  [INFO] Sensores nao confiaveis detectados - classificacao adaptativa sera usada")
            print(f"    Current confiavel: {'SIM' if self.sensores_confiaveis['current'] else 'NAO'}")
            print(f"    RPM confiavel: {'SIM' if self.sensores_confiaveis['rpm'] else 'NAO'}")
            print(f"    A analise continuara usando VIBRACAO e TEMPERATURA como indicadores principais")
        else:
            print("\n  [OK] Todos os sensores com cobertura adequada")
    ...
    def baixar_dados_influx(self):
        ...
        FALLBACK: Se o download falhar ou o arquivo nao for encontrado,
        tenta usar os arquivos gerais existentes em data/raw/ (sem tag de intervalo),
        filtrando pelo periodo solicitado e salvando uma copia com o tag do intervalo.
        """
        print("[DOWNLOAD] Baixando dados do InfluxDB...")

        # Mapeamento de tabelas para prefixos de arquivos gerais
        mapeamento_arquivos = {
            'estimated': f'dados_estimated_{self.mpoint}',
            'validated_default': f'dados_{self.mpoint}',
            'validated_slip': f'dados_slip_{self.mpoint}'
        }
        ...
        for script_nome, tabela in scripts_download:
            ...
            # Verificar se o arquivo com tag de intervalo ja existe
            prefixo = mapeamento_arquivos[tabela]
            arquivo_intervalo = self.dir_data_raw / f'{prefixo}_{self.intervalo_arquivo}.csv'
            ...
            if script_path.exists():
                try:
                    ...
                    result = subprocess.run(cmd, cwd=self.base_dir, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, encoding='utf-8', errors='replace')

                    if result.returncode == 0 and arquivo_intervalo.exists():
                        print("     Download concluido com sucesso")
                        download_ok = True
                        
                        # VALIDAR rotational_speed logo apos baixar estimated
                        if tabela == 'estimated':
                            self.validar_rotational_speed()
                    else:
                        if result.stdout:
                            print(f"     Saida: {result.stdout[:300]}")

                except Exception as e:
                    print(f"     Erro ao executar download: {e}")
            else:
                print(f"     Script {script_nome} nao encontrado")

            # ================================================================
            # FALLBACK: Usar arquivo geral existente e filtrar pelo intervalo
            # ================================================================
            if not download_ok and not arquivo_intervalo.exists():
                arquivo_geral = self.dir_data_raw / f'{prefixo}.csv'
                ...
                if arquivo_geral.exists():
                    ...
                    try:
                        ...
                        # Filtrar pelo intervalo solicitado (usar UTC)
                        dt_inicio = pd.to_datetime(self.data_inicio_utc, format='%Y-%m-%dT%H:%M:%SZ', utc=True)
                        dt_fim = pd.to_datetime(self.data_fim_utc, format='%Y-%m-%dT%H:%M:%SZ', utc=True)
                        
                        mask = (df_geral['time'] >= dt_inicio) & (df_geral['time'] <= dt_fim)
                        df_filtrado = df_geral[mask].copy()
                        
                        if len(df_filtrado) > 0:
                            # Salvar como arquivo com tag de intervalo
                            df_filtrado.to_csv(arquivo_intervalo, index=False)
                            print(f"     Filtrado: {len(df_filtrado)} registros de {len(df_geral)} totais")
                            ...
                            print(f"     [AVISO] Nenhum dado encontrado no intervalo {self.data_inicio} ate {self.data_fim}")
                            print(f"     Dados disponiveis: {df_geral['time'].min()} ate {df_geral['time'].max()}")
                    except Exception as e:
                        print(f"     [ERRO] Falha ao processar arquivo geral: {e}")
                else:
                    print(f"     [AVISO] Arquivo geral nao encontrado: {arquivo_geral.name}")
                    
                    # Verificar erro de conexao
                    if not download_ok:
                        erro_str = str(result.stderr) if 'result' in dir() and hasattr(result, 'stderr') else ''
                        erro_conexao = self._verificar_erro_conexao(erro_str)
                        if erro_conexao:
        ...
        for script_nome, descricao in scripts_processamento:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    print("     Erro no processamento (detalhes abaixo):")
                    print("-" * 40)
                    print(result.stdout)
                    print("-" * 40)
    ...
    def classificar_estados(self, df_normalizado, colunas_features):
        ...
        Respeita a confiabilidade dos sensores: quando current/RPM nao sao confiaveis,
        usa apenas vibracao e temperatura para classificar.
        """
        # Verificar confiabilidade dos sensores
        sensores = getattr(self, 'sensores_confiaveis', {'current': True, 'rpm': True})
        current_confiavel = sensores.get('current', True)
        rpm_confiavel = sensores.get('rpm', True)
        
        if not current_confiavel or not rpm_confiavel:
            print(" Classificando estados usando VIBRACAO e TEMPERATURA (current/RPM nao confiaveis)...")
        else:
        ...
        # Colunas para merge dependem da confiabilidade
        merge_cols = ['time', 'vel_rms']
        if current_confiavel:
            merge_cols.append('current')
        if rpm_confiavel:
            merge_cols.append('rotational_speed')
        # Sempre incluir temperatura se disponível
        merge_cols.append('object_temp')
        ...
        if arquivo_final.exists():
            ...
            merge_cols_existentes = [c for c in merge_cols if c in df_original.columns]

            # Merge para ter valores originais junto com normalizados
            df_completo = pd.merge(
                df_normalizado,
                df_original[merge_cols_existentes],
                on='time',
                how='left',
                suffixes=('_norm', '')
            )
        else:
            print("  [ERRO] Arquivo original nao encontrado - nao e possivel classificar sem dados reais")
            return None
        ...
        if hasattr(self, 'thresholds_desligado') and self.thresholds_desligado:
            ...
            if current_confiavel:
                ...
                print(f"    - current: IGNORADO (sensor nao confiavel)")
            
            if rpm_confiavel:
                rpm_max = self.thresholds_desligado.get('rpm_max', 100.0)  # RPM
                print(f"    - rpm_max: {rpm_max:.3f} RPM")
            else:
                print(f"    - rpm: IGNORADO (sensor nao confiavel)")

            for idx, row in df_completo.iterrows():
                vel_rms_real = row.get('vel_rms', 0)
                is_desligado = False
                
                if current_confiavel and rpm_confiavel:
                    # MODO COMPLETO: usar current + RPM + vibracao
                    ...
                    if current_real <= current_max and rpm_real <= rpm_max:
                        if vel_rms_real <= vel_rms_max:
                            is_desligado = True
                        elif vel_rms_real <= 3.0:
                            is_desligado = True
                    # Se current OU RPM acima do threshold = LIGADO
                else:
                    # MODO DEGRADADO: usar apenas vibracao (e temperatura se disponivel)
                    # Equipamento DESLIGADO = vibracao muito baixa
                    if vel_rms_real <= vel_rms_max:
                        is_desligado = True
                    # Vibracao alta = LIGADO (independente de current/RPM)
            ...
            print("  [ERRO] Thresholds do estado desligado nao disponiveis!")
    ...
    def gerar_visualizacao_3d_janela(self):
        """Executa visualização 3D em janela do Windows usando o script visualizar_clusters_3d_simples.py"""
        print(" Executando visualização 3D em janela do Windows...")
    ...
    def executar_analise_completa(self):
        ...
        try:
            ...
            try:
                ...
                plot_vibracao_reconstrucao_completa(self.mpoint, start_time=self.data_inicio, end_time=self.data_fim)
            except Exception as e:
...
def main():
    try:
        ...
        if not analisador.executar_analise_completa():
            sys.exit(1)
