...
def calcular_thresholds_estado_desligado(df_analise, clusters_desligado, vel_rms_cols, current_cols, rpm_cols, scaler=None, mpoint=None):
    ...
    try:
        DIR_RAW_PREENCHIDO = raw_preenchido_dir(mpoint)
...
def verificar_confiabilidade_sensores(mpoint):
    """
    Verifica a confiabilidade dos sensores de current e RPM analisando a proporcao
    de valores reais (nao-NaN) no arquivo estimated original.
    
    Se a proporcao de dados reais for < 30%, o sensor e considerado nao confiavel
    (nao estava calibrado/reportando) e nao deve ser usado para classificacao.
    
    LOGICA DE NEGOCIO: No sistema real, o equipamento so comeca a captar current
    e RPM se a corrente estiver calibrada no banco de dados relacional. Se nao
    estiver calibrada, os valores vem como NaN (nao captados), e NAO como zero.
    Tratar NaN interpolado como zero causaria classificacao errada.
    """
    confiabilidade = {
        'current_confiavel': True,
        'rpm_confiavel': True,
        'current_cobertura': 1.0,
        'rpm_cobertura': 1.0,
        'limiar_minimo': 0.30  # 30% minimo de dados reais
    }
    
    try:
        arquivo_estimated = BASE_DIR / 'data' / 'raw' / f'dados_estimated_{mpoint}.csv'
        if not arquivo_estimated.exists():
            if VERBOSE:
                print(f"  [AVISO] Arquivo estimated nao encontrado: {arquivo_estimated}")
                print(f"  [AVISO] Assumindo sensores confiaveis (fallback)")
        ...
        # Verificar current
        if 'current' in df_est.columns:
            current_validos = df_est['current'].notna().sum()
            confiabilidade['current_cobertura'] = current_validos / total if total > 0 else 0
            confiabilidade['current_confiavel'] = current_validos >= 1000
        else:
            confiabilidade['current_confiavel'] = False
            confiabilidade['current_cobertura'] = 0.0
        
        # Verificar rotational_speed
        if 'rotational_speed' in df_est.columns:
            rpm_validos = df_est['rotational_speed'].notna().sum()
            confiabilidade['rpm_cobertura'] = rpm_validos / total if total > 0 else 0
            confiabilidade['rpm_confiavel'] = rpm_validos >= 1000
        else:
            confiabilidade['rpm_confiavel'] = False
            confiabilidade['rpm_cobertura'] = 0.0
        
        if VERBOSE:
            print(f"\n  - Verificacao de confiabilidade dos sensores (arquivo estimated):")
            print(f"    Current: {current_validos} registros validos "
                  f"-> {'CONFIAVEL' if confiabilidade['current_confiavel'] else 'NAO CONFIAVEL (< 1000 registros)'}")
            print(f"    RPM: {rpm_validos} registros validos "
                  f"-> {'CONFIAVEL' if confiabilidade['rpm_confiavel'] else 'NAO CONFIAVEL (< 1000 registros)'}")
            
            if not confiabilidade['current_confiavel'] or not confiabilidade['rpm_confiavel']:
                print(f"    [IMPORTANTE] Sensores nao confiaveis serao IGNORADOS na classificacao")
                print(f"    [IMPORTANTE] Classificacao baseada em VIBRACAO e TEMPERATURA apenas")
                print(f"    [IMPORTANTE] Valores de current/RPM = 0 sao NaN interpolados, nao leituras reais")
    
    except Exception as e:
        if VERBOSE:
            print(f"  [AVISO] Erro ao verificar confiabilidade: {e}")
            print(f"  [AVISO] Assumindo sensores confiaveis (fallback)")
...
def classificar_2_estados_simples(df_analise, scaler=None, mpoint=None):
    ...
    CONFIABILIDADE: Verifica se sensores de current/RPM possuem dados reais suficientes.
    Se a cobertura for < 30%, esses sensores sao ignorados na classificacao
    (valores NaN interpolados como zero nao sao leituras reais).
    ...
    # Verificar confiabilidade dos sensores de current e RPM
    confiabilidade = verificar_confiabilidade_sensores(mpoint)
    ...
    for cluster_id in df_analise['cluster'].unique():
        ...
        # Pesos ajustados com base na confiabilidade dos sensores
        peso_vel = 1.0
        peso_current = 2.0 if confiabilidade['current_confiavel'] else 0.0
        peso_rpm = 2.0 if confiabilidade['rpm_confiavel'] else 0.0
        peso_temp = 0.5
        
        # Se current e RPM nao sao confiaveis, aumentar peso da vibracao e temperatura
        if not confiabilidade['current_confiavel'] or not confiabilidade['rpm_confiavel']:
            peso_vel = 3.0   # Vibracao passa a ser o indicador principal
            peso_temp = 1.5   # Temperatura ganha mais peso
        
        score = (vel_rms_mean_norm * peso_vel) + (current_mean_norm * peso_current) + (rpm_mean_norm * peso_rpm)
        
        # Score adicional para temperatura (equipamento desligado tende a estar mais frio)
        score += temp_mean_norm * peso_temp
    ...
    # ========================================================================
    # VERIFICAÇÃO FÍSICA: Clusters com current E rpm baixos
    # DEVEM ser DESLIGADO independente da vibração (vibrações residuais)
    # 
    # IMPORTANTE: Esta verificacao so e aplicada quando os sensores de
    # current e RPM sao CONFIAVEIS (>= 30% de dados reais no estimated).
    # Quando os sensores nao estao calibrados, os valores de current/RPM
    # sao NaN interpolados como zero, e NAO representam o estado real.
    # ========================================================================
    clusters_desligado_fisico = []
    
    if confiabilidade['current_confiavel'] and confiabilidade['rpm_confiavel']:
        # Sensores confiaveis: aplicar verificacao fisica normalmente
        for cluster_id, features in cluster_features.items():
            # Critério físico OBRIGATÓRIO:
            # Se (current_mean < 0.15 OR rpm_mean < 0.20) E vel_rms_mean_norm < 0.05
            # Isso força que current e rpm andem juntos. Se a corrente for zero, mas o rpm alucinar 1200 (ou vice-versa),
            # e a vibração for baixa, é classificado como DESLIGADO.
            if (features['current_mean_norm'] < 0.15 or features['rpm_mean_norm'] < 0.20) and features['vel_rms_mean_norm'] < 0.05:
                if cluster_id not in clusters_desligado:
                    ...
                    if VERBOSE:
                        ...
                        print(f"    Razão: Current baixa ({features['current_mean_norm']:.3f} < 0.15) OU RPM baixa ({features['rpm_mean_norm']:.3f} < 0.20)")
                        print(f"    Vibração: {features['vel_rms_mean_norm']:.3f} (residual - equipamento parado)")
                        print(f"    -> Equipamento FISICAMENTE DESLIGADO (corrente ou rpm nulos detectados)")
        ...
        # Sensores NAO confiaveis: pular verificacao fisica
        if VERBOSE:
            print(f"\n  - VERIFICAÇÃO FÍSICA IGNORADA:")
            if not confiabilidade['current_confiavel']:
                print(f"    Current: apenas {confiabilidade['current_cobertura']*100:.1f}% de dados reais (nao confiavel)")
            if not confiabilidade['rpm_confiavel']:
                print(f"    RPM: apenas {confiabilidade['rpm_cobertura']*100:.1f}% de dados reais (nao confiavel)")
            print(f"    Valores de current/RPM = 0 sao NaN interpolados, nao leituras reais")
            print(f"    Classificacao baseada apenas em vibracao e temperatura")

    # Resto = LIGADO
    # ADICIONADO: Proteção contra vibração alta (se > 0.20 norm, deve ser LIGADO)
    clusters_ligado = []
    for c in clusters_ordenados:
        cluster_id = c[0]
        if cluster_id in clusters_desligado:
            # Override de segurança: se a vibração for superior ao ruído base (0.06 norm), deve ser LIGADO
            if cluster_features[cluster_id]['vel_rms_mean_norm'] > 0.06:
                if VERBOSE:
                    print(f"  - OVERRIDE DE SEGURANÇA: Cluster {cluster_id} movido para LIGADO por vibração detectada ({cluster_features[cluster_id]['vel_rms_mean_norm']:.3f} > 0.06)")
                continue
            else:
                pass # Mantém como desligado
        
        if cluster_id not in clusters_desligado or cluster_features[cluster_id]['vel_rms_mean_norm'] > 0.06:
            clusters_ligado.append(cluster_id)
    
    # Recalcular clusters_desligado após override
    clusters_desligado = [c for c in clusters_desligado if c not in clusters_ligado]
...
def criar_visualizacoes_rigoroso(df_analise, colunas_validas, mpoint):
    """Cria visualizações dos resultados (modo rigoroso) com cores em tons de verde e vermelho"""
    ...
    # Identificar quais clusters pertencem a cada estado e seus scores médios para ordenar tons
    cluster_info = []
    for i in range(6):
        mask = df_analise['cluster'] == i
        if mask.any():
            status = df_analise.loc[mask, 'equipamento_status'].iloc[0]
            # Usar uma métrica simples (média das features) para ordenar a intensidade da cor
            score_medio = df_analise.loc[mask, colunas_validas].mean().mean()
            cluster_info.append({'id': i, 'status': status, 'score': score_medio})
    
    # Separar e ordenar
    clusters_desligado = sorted([c for c in cluster_info if c['status'] == 'DESLIGADO'], key=lambda x: x['score'])
    clusters_ligado = sorted([c for c in cluster_info if c['status'] == 'LIGADO'], key=lambda x: x['score'])
    
    # Mapeamento de cores
    cluster_colors = {}
    
    # Tons de Vermelho (mais escuro para scores menores/perto de 0)
    # Quanto menor o score, mais perto do início da lista clusters_desligado
    n_reds = len(clusters_desligado)
    red_palette = sns.light_palette("red", n_colors=n_reds + 2, reverse=True).as_hex()
    for i, c in enumerate(clusters_desligado):
        # i=0 (menor score) -> vermelho mais escuro
        cluster_colors[c['id']] = red_palette[i]
        
    # Tons de Verde (mais escuro para scores maiores/perto do valor máximo)
    # Quanto maior o score, mais perto do final da lista clusters_ligado
    n_greens = len(clusters_ligado)
    green_palette = sns.light_palette("green", n_colors=n_greens + 2, reverse=False).as_hex()
    for i, c in enumerate(clusters_ligado):
        # O último da lista (maior score) deve ser o mais escuro
        # i=0 (menor score ligado) -> verde mais claro
    ...
    output_path = dir_resultados / f'analise_kmeans_clusters_moderado{mpoint_tag}.png'
...
def salvar_modelos_e_dados_rigoroso(kmeans, scaler, df_analise, colunas_validas,
                                   ...
                                   thresholds_desligado=None, mpoint=None, tempo_execucao_min=None, tempo_execucao_seg=None):
    ...
    with open(relatorio_path, 'w', encoding='utf-8') as f:
        ...
        if tempo_execucao_min is not None:
            f.write(f"Tempo de execução: {tempo_execucao_min:.0f} minutos e {tempo_execucao_seg:.0f} segundos\n")
        f.write("\n")
...
def main():
    ...
    try:
        ...
        criar_visualizacoes_rigoroso(df_analise, colunas_validas, args.mpoint)

        # 8. Salvar modelos e dados
        print("\n[8/8] Salvando modelos e dados...", flush=True)
        
        tempo_total_segundos = time.time() - start_time_total
        tempo_total_minutos = tempo_total_segundos // 60
        tempo_restante_segundos = tempo_total_segundos % 60
        
        salvar_modelos_e_dados_rigoroso(kmeans, scaler, df_analise, colunas_validas,
                                       cluster_ligado, cluster_desligado, clusters_intermediarios,
                                       thresholds_desligado, args.mpoint, tempo_total_minutos, tempo_restante_segundos)

        print("\n" + "=" * 70)
        print("=== PROCESSO RIGOROSO CONCLUÍDO COM SUCESSO ===")
        print(f"Tempo de Execução: {tempo_total_minutos:.0f} minutos e {tempo_restante_segundos:.0f} segundos")
        print("=" * 70)
        ...
        # Log de visualização
        from utils.artifact_paths import results_dir
        dir_resultados = results_dir(args.mpoint, create=True) if args.mpoint else DIR_RESULTS
        chart_file = dir_resultados / f'analise_kmeans_clusters_moderado_{args.mpoint}.png'
        ...
        # [NOVO] Gerar gráfico de vibração temporal reconstruída completa após treino
        print("\n[EXTRA] Gerando gráfico de vibração temporal reconstruída completa...", flush=True)
        try:
            from scripts.plot_vibracao_temporal_colorido import plot_vibracao_reconstrucao_completa
            plot_vibracao_reconstrucao_completa(args.mpoint)
        except Exception as e:
            print(f"  [AVISO] Erro ao gerar gráfico temporal: {e}")
