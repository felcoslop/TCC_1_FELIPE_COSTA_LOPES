...
from utils.artifact_paths import raw_preenchido_dir
...
if __name__ == '__main__':
    ...
    dir_out = raw_preenchido_dir(args.mpoint, create=True)
    ...
    # Filtro de alucinação do magnetômetro (Interferência Eletromagnética)
    # Cenário: RPM sobe muito, mas a corrente não acompanha (aumento mínimo ou nulo).
    # Isso impede que o sistema classifique erroneamente como LIGADO.
    if 'rotational_speed' in df_est.columns and 'current' in df_est.columns:
        curr_validos = df_est.loc[df_est['current'] > 0, 'current']
        if len(curr_validos) > 100:
            curr_p90 = curr_validos.quantile(0.90)
            # Limite: 5% do P90 de corrente normal do equipamento, ou pelo menos 2.0A
            limite_corrente = max(curr_p90 * 0.05, 2.0)
            
            mask_alucinacao = (df_est['rotational_speed'] > 100) & (df_est['current'] < limite_corrente)
            qnt_alucinacao = mask_alucinacao.sum()
            
            if qnt_alucinacao > 0:
                print(f"   [FILTRO] Detectadas {qnt_alucinacao} amostras com alucinação de RPM/Corrente (Interferência EM).")
                print(f"            (RPM > 100, mas Corrente < {limite_corrente:.2f}A). Zerando valores para não interferir na classificação...")
                df_est.loc[mask_alucinacao, 'rotational_speed'] = 0.0
                df_est.loc[mask_alucinacao, 'current'] = 0.0
