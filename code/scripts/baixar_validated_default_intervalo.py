"""
Script pra baixar dados validados padrao do banco InfluxDB.
...
def main():
    parser = argparse.ArgumentParser(description="Baixar dados validated_default do InfluxDB")
    ...
    print(f" Processando dados validated_default para mpoint {mpoint}...")
    ...
    # Query COM filtro de intervalo, com agregação de 20s
    query = f"""
    SELECT
        MEAN("mag_x") AS "mag_x",
        MEAN("mag_y") AS "mag_y",
    ...
    if args.intervalo_arquivo:
        ...
        filename = dir_raw / f"dados_{mpoint}_{args.intervalo_arquivo}.csv"
    else:
        # Formato padrão
        ...
        filename = dir_raw / f"dados_{mpoint}_{inicio_str}_{fim_str}.csv"
