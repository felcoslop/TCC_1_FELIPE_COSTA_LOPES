...
def classificar_2_estados_mecanico(df_analise, mpoint=None):
    ...
    for cluster_id, features in cluster_features.items():
        # Temperatura próxima de ambiente (< 0.15 normalizado) E vibração próxima de zero (< 0.08 normalizado)
        if features['temp_mean_norm'] < 0.15 and features['vel_rms_mean_norm'] < 0.08:
    ...
    for c in clusters_ordenados:
        ...
        # Override de segurança: se a vibração for superior ao ruído base (0.08 norm), deve ser LIGADO
        if cluster_features[cluster_id]['vel_rms_mean_norm'] > 0.08:
            if VERBOSE:
                print(f"  - OVERRIDE DE SEGURANÇA: Cluster {cluster_id} movido para LIGADO por vibração detectada ({cluster_features[cluster_id]['vel_rms_mean_norm']:.3f} > 0.08)")
...
def criar_visualizacoes(df_analise, colunas_validas, mpoint):
    """Cria visualizações dos resultados com cores em tons de verde e vermelho"""
    ...
    axes[0,0].set_xlabel(f'PC1 ({pca.explained_variance_ratio_[0]:.2%} var)', fontsize=15, fontweight='bold')
    axes[0,0].set_ylabel(f'PC2 ({pca.explained_variance_ratio_[1]:.2%} var)', fontsize=15, fontweight='bold')
    axes[0,0].set_title('Clusters K-means - Equipamento MECANICO (6 clusters)', fontsize=13, fontweight='bold')
    ...
    axes[0,1].set_xlabel(f'PC1 ({pca.explained_variance_ratio_[0]:.2%} var)', fontsize=15, fontweight='bold')
    axes[0,1].set_ylabel(f'PC2 ({pca.explained_variance_ratio_[1]:.2%} var)', fontsize=15, fontweight='bold')
    axes[0,1].set_title('Classificacao - Temperatura + Vibracao', fontsize=13, fontweight='bold')
    ...
    axes[1,0].set_xlabel('Cluster', fontsize=15, fontweight='bold')
    axes[1,0].set_ylabel('Numero de Amostras (k = 1000)', fontsize=15, fontweight='bold')
    axes[1,0].set_title('Distribuicao dos Clusters', fontsize=13, fontweight='bold')
    ...
    # 4. Distribuição do status — labels negrito/maior, % maior com fundo branco
    ...
    if len(status_counts) > 0:
        ...
        _wedges, _texts, _autotexts = axes[1,1].pie(
            ...
            colors=pie_colors, startangle=140, explode=[0.03]*len(status_counts),
        ...
        axes[1,1].axis('equal')
    axes[1,1].set_title('Distribuicao do Status', fontsize=13, fontweight='bold')
    ...
    _l1m = fig.legend(handles=_leg1_m,
    ...
    output_path = dir_resultados / f'analise_kmeans_clusters_mecanico{mpoint_tag}.png'
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  - Visualização salva em: {output_path}")

def salvar_modelos_e_dados(kmeans, scaler, df_analise, colunas_validas, thresholds_desligado, mpoint, tempo_execucao_min=None, tempo_execucao_seg=None):
...
def main():
    ...
    try:
        ...
        salvar_modelos_e_dados(kmeans, scaler, df_analise, colunas_validas, thresholds_desligado, args.mpoint, tempo_total_minutos, tempo_restante_segundos)
