...
class SegmentadorPreenchedor:
    ...
    def interpolar_avancado(self, df, colunas_numericas):
        ...
        # ETAPA 1: Interpolação PCHIP para Gaps Médios (até 30 minutos / 90 amostras)
        # O PCHIP é mais robusto que a Spline Cúbica e evita oscilações artificiais (overshoot)
        print("  --> Etapa 1/3: Interpolação PCHIP (Gaps <= 30 min)...")
        for col in colunas_sensoriais:
            if col in df_interpolado.columns:
                # O limite de 90 amostras garante que apenas gaps <= 30min sejam totalmente preenchidos aqui
                # Gaps maiores serão deixados para o KNN (Etapa 2)
        ...
        # ETAPA 2: KNN Multivariado para Gaps Longos (30 min até 3 horas)
        # Usa a relação entre sensores (Corrente, RPM, etc) para reconstruir a assinatura da máquina
        print("  --> Etapa 2/3: KNN multivariado (Gaps > 30 min e <= 3h)...")
        ...
        # ETAPA 3: Preenchimento de extremidades (Forward/Backward fill)
        print("  --> Etapa 3/3: Ajustes finais de extremidades...")
        for col in colunas_sensoriais:
            if col in df_interpolado.columns:
