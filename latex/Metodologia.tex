...
\begin{table}[H]
    ...
    \begin{tabular}{cccccc}
        \hline
        \textbf{Time (UTC)} & \textbf{object\_temp} & \textbf{vel\_rms\_x} & \textbf{current} & \textbf{rotational\_speed} & \textbf{fe\_frequency} \\
        \hline
        10:00:00 & 45,2 & 2,1 & 12,5 & 1180 & 1,85 \\
        10:00:20 & 45,3 & 2,2 & 12,6 & 1182 & 1,85 \\
        10:00:40 & 45,3 & 2,1 & 12,6 & 1182 & \textit{NaN} \\
        10:01:00 & 45,4 & 2,3 & 12,6 & 1182 & \textit{NaN} \\
        \hline
    \end{tabular}
\end{table}
...
\begin{enumerate}
    \item \textbf{Validação de requisitos:} Verifica a existência do modelo K-Means treinado e do \textit{scaler} MinMax persistidos para o equipamento, além de testar a conectividade com o InfluxDB e exigir um intervalo de análise de no mínimo 24 horas.
    \item \textbf{Extração e validação:} Converte as marcações de horário fornecidas pelo operador (GMT-3) para UTC, efetua o \textit{download} das séries brutas e valida a densidade do intervalo, exigindo cobertura de dados maior que 70\% e rejeitando períodos com lacunas consecutivas maiores que 10 minutos (valores 0,0 são válidos; o que invalida são ausências reais de sinal).
    \item \textbf{Processamento:} Realiza a segmentação temporal, interpolação adaptativa de lacunas e a sincronização multifrequência.
    \item \textbf{Classificação avançada:} Aplica o classificador K-Means nos dados normalizados e realiza correções usando os limiares físicos salvos durante a etapa de treinamento. Não há necessidade de re-treinar o modelo ou recalcular limiares; a classificação utiliza a lógica de máxima cautela descrita na Seção~\ref{sec:kmeans_classification}, comparando valores físicos reais (RPM, corrente e vibração). Por fim, aplica-se o filtro de \textit{outliers} temporais para remover transições espúrias com duração menor que 10 minutos.
    \item \textbf{Análise de métricas:} Calcula e reporta o tempo acumulado em cada estado operacional (LIGADO, DESLIGADO, e Incerteza/Sem Conexão) com base nas durações temporais reais, gerando relatórios textuais e gráficos de reconstrução temporal.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Seleção de Intervalo Otimizado:} O sistema emprega um algoritmo de busca para identificar e recortar o "melhor período" de dados contínuos. Este intervalo representativo é escolhido de forma que apresente uma distribuição equilibrada entre os estados LIGADO e DESLIGADO, além de maximizar a presença de transições entre os estados e garantir que a densidade e a qualidade do sinal sejam ideais.
    \item \textbf{Downsampling Sistemático:} Sobre o intervalo selecionado, os pontos são amostrados sistematicamente (por exemplo, aplicando um fator de passo para extrair uma amostra a cada $N$ registros), reduzindo a quantidade de elementos gráficos a um volume ótimo para renderização instantânea (tipicamente limitado a cerca de 10.000 pontos).
\end{itemize}
...
\begin{itemize}
    \item \textbf{Equipamentos Elétricos:} São gerados dois gráficos distintos. O primeiro utiliza os eixos Corrente (A) $\times$ Vibração RMS (mm/s) $\times$ Tempo. O segundo utiliza RPM $\times$ Vibração RMS (mm/s) $\times$ Tempo. Esses gráficos buscam relacionar os sinais elétricos estimados com a resposta física de vibração ao longo da série histórica.
    \item \textbf{Equipamentos Mecânicos:} Como não há sensores elétricos, o sistema gera o gráfico utilizando Temperatura ($^\circ$C) $\times$ Vibração RMS (mm/s) $\times$ Tempo, mostrando a dependência térmica inerente ao regime mecânico operacional.
\end{itemize}
