...
Caso restem lacunas após essa fusão (como os valores \textit{NaN} de \texttt{fe\_frequency} nos instantes finais devido ao esgotamento da janela de retropropagação de 3 minutos), estas são tratadas pelas etapas subsequentes de interpolação adaptativa descritas na Seção~\ref{sec:processamento_interpolacao}.
...
O sistema aplica uma coloração binária automática nos pontos baseando-se na classificação dos clusters (verde para LIGADO e vermelho para DESLIGADO) e salva automaticamente a imagem estática no diretório \texttt{results/}. Uma explanação profunda de cada um dos gráficos tridimensionais históricos e de intervalos gerados por esta aba do sistema, acompanhada da devida análise física, é detalhada no Capítulo~\ref{chap:resultados} (Resultados) nas Figuras 4.1, 4.7, etc.
