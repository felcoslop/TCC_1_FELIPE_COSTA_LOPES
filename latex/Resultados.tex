...
A Tabela~\ref{tab:resultados_classificacao} sintetiza os resultados obtidos para cada equipamento, detalhando o tipo, descrição, número de amostras processadas e a quantidade de atributos utilizados na classificação.
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|c|r|c|}
        \hline
        \textbf{Equipamento} & \textbf{Tipo} & \textbf{Descrição} & \textbf{Amostras}  & \textbf{Atributos} \\
        \hline
        $c_{636}$            & Elétrico      & Motobomba          & 863.450            & 19                 \\
        $c_{637}$            & Elétrico      & Motobomba          & 1.242.993          & 19                 \\
        $c_{640}$            & Elétrico      & Motor REC          & 1.389.174          & 19                 \\
        $c_{1518}$           & Mecânico      & Mancal             & 585.367            & 16                 \\
        $c_{670}$            & Mecânico      & Redutora bridle    & 1.377.884          & 16                 \\
        \hline
        \textbf{Total}       & \textbf{---}  & \textbf{---}       & \textbf{5.458.868} & \textbf{---}       \\
        \hline
    \end{tabular}
\end{table}

Conforme apresentado na Tabela~\ref{tab:resultados_classificacao}, todos os cinco equipamentos foram classificados integralmente, com 100\% dos pontos rotulados em LIGADO ou DESLIGADO. Vale ressaltar que a classificação não é avaliada por uma métrica de acurácia supervisionada: como o estado é determinado pela comparação dos dados com \textit{thresholds} físicos calculados a partir dos próprios \textit{clusters}, todo ponto é necessariamente atribuído a um dos estados, de modo que a cobertura é sempre total por construção. O total de amostras processadas ultrapassa 5,4 milhões de registros, validando a escalabilidade do sistema.
...
O algoritmo K-Means foi executado com $k = 6$ \textit{clusters} para todos os equipamentos, mostrando convergência robusta. A escolha de $k = 6$ foi motivada pela necessidade de representar a proporção aproximada de 1 para 2 entre os estados desligado e ligado, reservando parte dos \textit{clusters} para capturar estados de transição (desligando ou ligando, ou seja, estados intermediários entre os dois extremos). Concebeu-se inicialmente a reserva de dois \textit{clusters} para o estado DESLIGADO; na lógica final de classificação, porém, apenas o \textit{cluster} de menor \textit{score} é atribuído como DESLIGADO por padrão, admitindo-se um segundo \textit{cluster} somente quando atendidos os critérios estritos de similaridade descritos na Seção~\ref{sec:kmeans_classification}. Os \textit{clusters} restantes representam diferentes regimes de operação LIGADO sob cargas variáveis. Essa configuração assegura que o sistema não confunda estados transitórios com estados estacionários, melhorando a robustez da classificação final.
...
Esse padrão pode ser verificado de forma interativa no visualizador web das séries temporais classificadas, disponível em \url{https://evolution-tcc-felipe-costa-lopes-visualizador.hfzba0.easypanel.host/}, onde é possível navegar pelas janelas temporais e observar os ciclos curtos de ativação e desativação do equipamento ao longo de todo o período histórico.
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|r|r|r|}
        \hline
        \textbf{Método}            & \textbf{Pontos} & \textbf{Horas}  & \textbf{Qualidade} \\
        \hline
        PCHIP ($\leq$ 30 min)      & 25.514          & 141,74          & 0,95               \\
        KNN (30 min -- 3h)         & 2.896           & 16,09           & 0,75               \\
        \hline
        \textbf{Total interpolado} & \textbf{28.410} & \textbf{157,83} & \textbf{---}       \\
