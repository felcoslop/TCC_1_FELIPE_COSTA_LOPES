...
\section{Interpolação Adaptativa de Lacunas}
\label{sec:resultados_interpolacao}

A Tabela~\ref{tab:interpolacao_horas} apresenta, para cada equipamento analisado, o volume de dados reconstruídos pela interpolação adaptativa, discriminado entre os métodos PCHIP (lacunas $\leq$ 30 minutos) e KNN multivariado (lacunas entre 30 minutos e 3 horas), em contraste com os dados originais do sensor. A coluna Qualidade corresponde ao indicador de confiabilidade atribuído a cada amostra conforme sua origem, definido na Seção~\ref{chap:metodologia} (Processamento e Interpolação): 1,00 para dados originais do sensor, 0,95 para valores reconstruídos por PCHIP e 0,75 para valores reconstruídos por KNN. Trata-se de um peso de confiança decrescente que reflete a incerteza progressivamente maior associada à interpolação, sendo máximo para a leitura direta do sensor e reduzido à medida que a lacuna preenchida é maior e o método de estimativa, menos direto.

\begin{table}[H]
    \centering
    \caption{Horas interpoladas por método e por equipamento}
    \label{tab:interpolacao_horas}
    \begin{tabular}{|l|r|r|r|r|r|}
        \hline
        \textbf{Equipamento} & \textbf{Originais (h)} & \textbf{PCHIP (h)} & \textbf{KNN (h)} & \textbf{Total interp. (h)} & \textbf{Interp. (\%)} \\
        \hline
        $c_{636}$ (El.) & 4.251,2 & 533,8 & 12,0  & 545,8  & 11,4 \\
        $c_{637}$ (El.) & 6.747,7 & 141,7 & 16,1  & 157,8  &  2,3 \\
        $c_{640}$ (El.) & 7.547,6 & 153,5 & 16,5  & 170,0  &  2,2 \\
        $c_{1518}$ (Mec.) & 2.860,1 & 342,6 & 49,4  & 391,9  & 12,1 \\
        $c_{670}$ (Mec.)  & 6.463,1 & 948,7 & 243,1 & 1.191,8 & 15,6 \\
        \hline
    \end{tabular}
\end{table}

Os resultados revelam comportamentos distintos entre os equipamentos. Os elétricos $c_{637}$ e $c_{640}$ apresentaram as menores proporções de interpolação (2,3\% e 2,2\%, respectivamente), indicando séries temporais com poucas lacunas. O $c_{636}$ demandou reconstrução mais expressiva (11,4\%), com a quase totalidade via PCHIP, o que sugere lacunas frequentes mas curtas. Nos mecânicos, o $c_{670}$ (redutora) exigiu o maior volume de interpolação absoluto (1.191,8 horas, 15,6\%), com proporção relevante de KNN (243,1 horas), sinalizando lacunas de maior duração. Em todos os casos, o método KNN foi aplicado em volume inferior ao PCHIP, o que reflete a menor frequência de lacunas de duração intermediária (30 minutos a 3 horas) em comparação às lacunas curtas.
...
O algoritmo K-Means foi executado com $k = 6$ \textit{clusters} para todos os equipamentos, mostrando convergência robusta. A escolha de $k = 6$ foi motivada pela necessidade de representar a proporção aproximada de 1 para 2 entre os estados desligado e ligado, reservando parte dos \textit{clusters} para capturar estados de transição (desligando ou ligando, ou seja, estados intermediários entre os dois extremos). Por concepção, o algoritmo K-Means com $k=6$ avalia sistematicamente os dois \textit{clusters} de menor \textit{score} como candidatos ao estado DESLIGADO em todos os equipamentos, conforme a reserva intencional descrita na Seção~\ref{sec:kmeans_classification}. Após a clusterização, ambos os candidatos passam por uma etapa de validação com critérios estritos (diferença de \textit{score} $<$ 0,3, ambos com \textit{score} $<$ 0,5 e vibração residual dentro do limiar). Na prática, o segundo candidato quase sempre falha na validação, revelando um regime de baixa carga operacional e não inatividade real, sendo então reclassificado como LIGADO. O evento raro é que o segundo candidato \textit{sobreviva} à validação, confirmando dois perfis distintos de repouso; o restante dos \textit{clusters} representa diferentes regimes LIGADO sob cargas variáveis. A Figura~\ref{fig:kmeans_2desligado} ilustra justamente esse caso no equipamento $c_{637}$, mostrando o momento da convergência (iteração~11) em que dois centróides de baixa atividade ainda se apresentam separados antes da validação final.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.58\textwidth]{plots/03_kmeans_animacao.png}
    \caption{Convergência do K-Means (K = 6) na iteração 11 --- equipamento $c_{637}$ (projeção PCA 2D). Dois centróides de baixa atividade emergem durante a convergência: \textit{Desligado C1} (vermelho sólido, menor \textit{score}) e \textit{Desligado C2} (vermelho claro). Na classificação final do \textit{pipeline}, o segundo agrupamento (\textit{Desligado C2}) não satisfaz os critérios estritos de similaridade com C1 --- diferença de \textit{score} $\geq 0{,}3$ ou \textit{score} acima de $0{,}5$ --- indicando que C2 captura um regime de operação em baixa carga, e não inatividade verdadeira, sendo portanto reclassificado como LIGADO. A figura exibe a iteração 11, na qual os dois padrões de baixa atividade ainda estão separados, servindo como ilustração do mecanismo que motiva a reserva de dois \textit{clusters} para o estado DESLIGADO na concepção do sistema.}
    \label{fig:kmeans_2desligado}
\end{figure}

As inércias finais variaram entre cerca de 127 e 178 mil, indicando que os \textit{clusters} formados apresentaram baixa dispersão interna, ou seja, os pontos dentro de cada \textit{cluster} estavam próximos entre si. O tempo de processamento foi de 1 a 2 minutos por equipamento, utilizando um notebook Dell equipado com processador Intel Core i3 de 13\textsuperscript{a} geração e 64 GB de memória RAM. A reprodutibilidade dos resultados foi garantida através de \textit{random state} controlado. A classificação dinâmica baseada em \textit{scores} combinados identificou automaticamente \textit{clusters} LIGADO e DESLIGADO.
...
\begin{table}[H]
    ...
    \begin{tabular}{lcc}
        \hline
        \textbf{Equipamento} & \textbf{DESLIGADO (\%)} & \textbf{LIGADO (\%)} \\
        \hline
        $c_{636}$            & 4,75                    & 95,25                \\
        $c_{637}$            & 4,52                    & 95,48                \\
        $c_{640}$            & 5,64                    & 94,36                \\
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{results/estados_corrente_vibracao_tempo_3d_c_636.png}
...
A Figura~\ref{fig:3d_rpm_c636} mostra o espaço RPM $\times$ Vibração $\times$ Tempo para o mesmo equipamento $c_{636}$ e constitui um caso de exceção de comportamento, em contraste com a coerência física observada nos demais equipamentos. Nela, os pontos classificados como LIGADO (verde) concentram-se em RPM elevado (próximo de 1200) com vibração praticamente nula, enquanto os pontos DESLIGADO (vermelho) situam-se em RPM próximo de zero. Esse padrão não reflete a correlação física esperada entre velocidade rotacional e vibração, pois uma máquina efetivamente girando a 1200 RPM deveria apresentar vibração não desprezível. Trata-se de uma alucinação persistente na inferência de RPM e corrente a partir do fluxo magnético do sensor, fenômeno descrito na Seção~\ref{sec:processamento_interpolacao} (Interferência Eletromagnética). Diferentemente dos picos transitórios, neste equipamento o sensor reportou de forma persistente valores espúrios e simultâneos de RPM e corrente, de modo que a trava anti-EMI --- que zera as leituras quando a corrente cai abaixo do limiar dinâmico $\max(P90 \times 5\%,\ 2{,}0\ \text{A})$ --- não chegou a ser acionada. Como o \textit{score} elétrico pondera fortemente RPM e corrente (peso $2,0$), o estado LIGADO acabou sendo determinado por esses sinais estimados, e não pela vibração. Registra-se, portanto, esta figura como uma exceção atribuível a artefato de instrumentação, e não como evidência da correlação física entre rotação e vibração.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{results/estados_rpm_vibracao_tempo_3d_c_636.png}
    \caption{RPM $\times$ Vibração $\times$ Tempo -- $c_{636}$, período histórico. Caso de exceção: pontos LIGADO (verde) em RPM próximo de 1200 com vibração praticamente nula, decorrentes de uma alucinação persistente na inferência de RPM e corrente (Interferência Eletromagnética), e não da correlação física esperada entre rotação e vibração.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.50\textwidth]{results/estados_corrente_vibracao_tempo_3d_c_640.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.50\textwidth]{results/estados_rpm_vibracao_tempo_3d_c_640.png}
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|l|}
        \hline
        \textbf{Método de Validação} & \textbf{Observações}                  \\
        \hline
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|c|c|c|c|}
        \hline
        \textbf{Etapa}  & \textbf{$c_{636}$} & \textbf{$c_{637}$} & \textbf{$c_{640}$} & \textbf{$c_{1518}$} & \textbf{$c_{670}$} \\
        \hline
        Segmentação$^{*}$ & 0,51             & 1,08               & 4,68               & \multicolumn{2}{c|}{incorporada ao processamento} \\
    ...
    \smallskip
    \footnotesize{$^{*}$ No \textit{pipeline} mecânico ($c_{1518}$ e $c_{670}$), a segmentação temporal é realizada internamente pelo script de processamento, sem etapa dedicada separada; por isso o tempo está consolidado na linha Processamento.}
\end{table}

Os resultados demonstram que o \textit{pipeline} completo processa cada equipamento em menos de 14 minutos, mesmo com mais de 1,3 milhão de amostras. A etapa de K-Means é consistentemente a mais custosa (1,9 a 3,5 minutos), seguida pela normalização. O tempo total para processar todos os cinco equipamentos foi de aproximadamente 44,4 minutos.
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|r|c|c|c|}
        \hline
        \textbf{Equipamento} & \textbf{Tipo} & \textbf{Amostras}  & \textbf{Períodos} & \textbf{Válidos} & \textbf{Eficiência} \\
        \hline
...
Neste contexto, a eficiência de processamento é definida como a razão entre o número de períodos válidos (aqueles aprovados em todos os critérios de qualidade) e o número total de períodos identificados na segmentação temporal, expressa em percentual. Por exemplo, o equipamento $c_{636}$ teve 5 períodos válidos dentre os 12 segmentados, resultando em eficiência de $5/12 = 41{,}7\%$. Trata-se, portanto, de uma medida da proporção de dados que sobrevive aos filtros de qualidade na fase de treinamento, e não de uma métrica de desempenho computacional. A eficiência de processamento variou de 31,8\% ($c_{1518}$) a 58,5\% ($c_{670}$). A rejeição automática de períodos com lacunas superiores a 3 horas ou duração insuficiente (\textless 24 horas) é aplicada exclusivamente durante a fase de treinamento, garantindo que apenas dados de alta qualidade e ciclos operacionais completos alimentem a construção do modelo K-Means. Já na etapa de classificação de intervalos, o sistema processa 100\% dos pontos do período solicitado (respeitando apenas a segmentação por lacunas), priorizando a reconstrução histórica total após o modelo estar consolidado.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{results/vibracao_reconstrucao_c_636.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{results/vibracao_reconstrucao_c_637.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{results/vibracao_reconstrucao_c_640.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{results/vibracao_reconstrucao_c_1518.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{results/vibracao_reconstrucao_c_670.png}
...
Conforme evidenciado na Figura~\ref{fig:vibracao_c_670}, o equipamento $c_{670}$ (Bridle~4~R1 --- redutora) apresenta um padrão operacional distinto dos demais: transições frequentes e de curta duração entre os estados LIGADO e DESLIGADO. Diferentemente das motobombas e do mancal, que operam de forma quase contínua, a redutora \textit{bridle} tem seu funcionamento intrinsecamente condicionado à carga da linha de laminação. Esse equipamento atua sob demanda, sendo acionado apenas durante a passagem efetiva da tira pelo laminador e desligando-se nos intervalos entre bobinas ou durante pausas operacionais. Tal comportamento cíclico, com períodos LIGADO relativamente curtos intercalados por desligamentos frequentes, é característico de equipamentos mecânicos cuja operação depende diretamente do fluxo de produção da planta, e não de um regime contínuo. A proporção de 21\% em estado DESLIGADO --- a maior entre todos os equipamentos analisados --- corrobora essa hipótese de funcionamento intermitente condicionado à carga.

Esse padrão pode ser verificado de forma interativa no visualizador web das séries temporais classificadas, conforme detalhado na Seção~\ref{sec:disponibilizacao} (Disponibilização do Código), onde é possível navegar pelas janelas temporais e observar os ciclos curtos de ativação e desativação do equipamento ao longo de todo o período histórico.
